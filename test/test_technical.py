...
class TestTechnical(unittest.TestCase):
    ...
    def test_liquidity(self):
        ...
        daily_data = self.technical_data[
            (self.technical_data["tickersymbol"] == firm)
            & (self.technical_data["date"] < test_day)
        ].iloc[-20:]
        ...
        liquidity = liquidity[
            (liquidity["tickersymbol"] == firm) & (liquidity["date"] == test_day)
        ]
        ...
        for signal in signals:
            ...
            if sig_type == "liquidity":
                pd_testing.assert_frame_equal(
                    technical.liquidity(20, None, None), technical.get_signal(signal)
                )

            elif sig_type == "rsi":
                pd_testing.assert_frame_equal(
                    technical.ewm_rsi(20, None, None), technical.get_signal(signal)
                )
    ...
    def test_filter_signal(self):
        ...
        merged = merging(
            [liquidity, rsi], columns=["year", "quarter", "date", "tickersymbol"]
        )

        filtered = technical.filter_signal(
            signals=[("liquidity", 20, 1e6, 5e6), ("rsi", 20, 0.6, 0.7)]
        )
