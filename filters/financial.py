...
class FinancialSignal:
    ...
    def eps(self):
        owner_capital = self.data[self.data["code"] == 4110].copy().rename(columns={"value": "outstanding_share"}).drop(columns=['code'])
        earning = self.data[self.data["code"] == 72].copy().rename(columns={"value": "earning"}).drop(columns=['code'])

        owner_capital["outstanding_share"] = owner_capital["outstanding_share"].copy() / 10000

        eps = pd.merge(earning, owner_capital, on=["year", "quarter", "tickersymbol"]).sort_values(by=["year", "quarter", "tickersymbol"])
    ...
    def quick_ratio(self):
        ...
        quick_ratio = pd.merge(quick_ratio, investment, on=["year", "quarter", "tickersymbol"], how='left').sort_values(by=["year", "quarter", "tickersymbol"])
    ...
    def gross_margin(self):
        revenue = self.data[self.data["code"] == 10].copy().rename(columns={"value": "revenue"}).drop(columns=['code'])
        profit = self.data[self.data["code"] == 30].copy().rename(columns={"value": "profit"}).drop(columns=['code'])

        gross_margin = pd.merge(profit, revenue, on=["year", "quarter", "tickersymbol"]).sort_values(by=["year", "quarter", "tickersymbol"])
        gross_margin["gm"] = gross_margin["profit"].copy() / gross_margin["revenue"].copy()
    ...
    def roe(self):
        profit_after_tax = self.data[self.data["code"] == 70].copy().rename(columns={"value": "profit"}).drop(columns=['code'])
        equity = self.data[self.data["code"] == 4100].copy().rename(columns={"value": "equity"}).drop(columns=['code'])

        roe = pd.merge(profit_after_tax, equity, on=["year", "quarter", "tickersymbol"]).sort_values(by=["year", "quarter", "tickersymbol"])
    ...
    def turnover_inventory(self):
        cogs = self.data[self.data["code"] == 21].rename(columns={"value": "cogs"}).drop(columns=['code'])
        inventory = self.data[self.data["code"] == 1400].rename(columns={"value": "inv"}).drop(columns=['code'])

        turnover_inventory = pd.merge(cogs, inventory, on=["year", "quarter", "tickersymbol"]).sort_values(by=["year", "quarter", "tickersymbol"])
        turnover_inventory["turnover-inv"] = turnover_inventory["cogs"].copy() / turnover_inventory["inv"].copy()
