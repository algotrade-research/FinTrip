...
class Backtesting:
        ...
        for d in self.inv:
            for index, firm in enumerate(self.inv[d]):
                ...
                if firm["counter"] == 1:
                    ...
                    self.unrealized_assets[d] += (
                        unrealized_pnl - self.sell_fee * realized_pnl
                    )
    ...
    def _clear_inv(self):
        ...
        for firm in self.end:
            ...
            new_inv = [
                date_inv[index]
                for index in range(len(date_inv))
                if index != firm["index"]
            ]
        ...
        if no_stock >= 1:
            ...
            self.assets.append(
                [
                    ...
                    no_stock,
                    price,
                    self.unrealized_assets[buying_date],
                ]
            )

            self.inv[buying_date].append(
                {
    ...
    def price(self):
        ...
        prices = (
            pd.merge(portfolio, prices, on=["date", "tickersymbol"], how="right")
            .replace({np.nan: None})
            .sort_values(by=["date", "tickersymbol"])
        )
