...
if __name__ == "__main__":
    ...
    technical_factors = technical_signal.filter_signal(
        [
            (
                "liquidity",
                backtesting_config["liquidity_look_back"],
                backtesting_config["liquidity_lb"],
                backtesting_config["liquidity_ub"],
            ),
            (
                "rsi",
                backtesting_config["rsi_look_back"],
                backtesting_config["rsi_lb"],
                backtesting_config["rsi_ub"],
            ),
        ]
    )
    ...
    for key in keys:
        ...
        financial_factors = (
            financial_signal.filter_median([key])
            if key != "combine"
            else financial_signal.filter_median(backtesting_config["combination"])
        )

        top = backtesting_config["no_stock"]
        signal_factors = merging(
            [technical_factors, financial_factors],
